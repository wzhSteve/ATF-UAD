...
class ATF_UAD(nn.Module):
	def __init__(self, feats):
		super(ATF_UAD, self).__init__()
		self.name = 'ATF_UAD'
		self.n_window = 10
		self.n_feats = feats
		self.d_model = 64
		...
		# Reconstructor
		self.reconstructor = TimeReconstructor(self.n_window, self.n_feats, self.d_model, step_size=2)
		self.frequency_rec = FrequenceReconstructor(self.n_window, self.n_feats, self.d_model, step_size=2)
		self.alpha = nn.Parameter(torch.ones(1, 1, self.n_feats))
		self.sigma = 0.01
		self.beta = beta
		self.re_linear = nn.Linear(self.n_window, 1)

	def forward(self, current):
		# x: [batch_size, window, dim]
		# pred = self.predictor(past)
		# rec1 = self.reconstructor(pred.detach())
		# rec, output_list = self.reconstructor(current.permute(1, 0, 2))
		fre_rec = self.frequency_rec(current.permute(1, 0, 2))
		time_rec, output_list = self.reconstructor(current.permute(1, 0, 2))
		output = (1-self.beta) * time_rec + self.beta * self.alpha * fre_rec
		output = self.re_linear(output.permute(0, 2, 1)).permute(0, 2, 1)
		return output.permute(1, 0, 2), [fre_rec.permute(1, 0, 2), time_rec.permute(1, 0, 2)]  # [batch_size, window, dim]